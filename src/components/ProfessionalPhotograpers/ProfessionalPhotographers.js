import React from "react";
import photographerPositions from "../../lib/data/photographerPositions";

const ProfessionalPhotographers = () => {
	return (
		<div className="max-w-7xl mx-auto px-2 sm:px-6 lg:px-8 my-16">
			<div className="text-center my-16">
				<h1 className="text-primary text-3xl sm:text-4xl font-semibold text-gray-700 mb-3">
					{/* We focus on the best quality */}
					We have professional photographers
				</h1>
				<p className="text-primary text-md">
					We have professional photographers. So, don't worry about picture
					quality. <br /> Also, have big budget light facilities for an awesome
					photo
				</p>
			</div>
			{/* photographers position*/}
			{photographerPositions.map((position) => (
				<div key={position.id} className="flex items-center justify-center">
					<div className={`order-${position.order}`}>
						<img className="h-72" src={position.image} alt={position.title} />
					</div>
					<div className="sm:w-1/2 border shadow p-3 rounded">
						<h3 className="text-xl text-primary font-medium my-3">
							{position.title}
						</h3>
						<p className="text-primary text-md">{position.details}</p>
					</div>
				</div>
			))}
		</div>
	);
};

export default ProfessionalPhotographers;
